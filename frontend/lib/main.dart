...
Future<void> main() async {
  ...
  ErrorWidget.builder = (details) {
    return const Material(
      ...
      child: Center(
        child: Text(
          'StaySmart dang gap loi hien thi. Vui long tai lai trang.',
      ...
      builder: (context, _) {
        return MaterialApp(
          ...
          onGenerateRoute: (settings) {
            ...
            return _safeFallbackRoute(settings);
          },
          onUnknownRoute: _safeFallbackRoute,
        );
      },
    );
  }

  Route<dynamic> _safeFallbackRoute(RouteSettings settings) {
    return MaterialPageRoute(
      settings: settings,
      builder: (context) {
        final isCustomer = ApiService().currentSession?.isCustomer ?? false;
        return isCustomer ? const HomeScreen() : const WelcomeScreen();
      },
    );
  }
}
