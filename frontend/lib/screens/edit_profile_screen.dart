...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      var avatarUrl = _avatarUrlController.text;
      final pendingAvatarBytes = _pendingAvatarBytes;
      final pendingAvatarFileName = _pendingAvatarFileName;
      if (pendingAvatarBytes != null && pendingAvatarFileName != null) {
        setState(() {
          _isUploadingAvatar = true;
        });
        avatarUrl = await ApiService().uploadCurrentUserAvatar(
  ...
  Future<void> _pickAvatar() async {
    if (!ApiService().isAuthenticated || _isSaving) return;
    ...
    if (bytes.length > AppConstants.maxAvatarUploadBytes) {
      setState(() {
        _errorMessage = 'Anh dai dien khong duoc vuot qua 25MB.';
      });
      return;
    }

    setState(() {
      _pendingAvatarBytes = Uint8List.fromList(bytes);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Da chon anh. Bam Luu thay doi de cap nhat ho so.'),
      ),
    );
  }
        ...
        children: [
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              label: Text(
                _isUploadingAvatar ? 'Dang upload anh' : 'Chon anh tu thiet bi',
              ),
