...
class ApiService {
  ...
  Future<Map<String, dynamic>> sendAiChatMessage({
    ...
    try {
      final activeThreadId = await _ensureAiChatThread(threadId);
      final data = await _postRaw(
        ...
        timeout: AppConstants.aiChatTimeout,
      );
  ...
  Future<String> _ensureAiChatThread(String? threadId) async {
    final existingThreadId = threadId?.trim();
    ...
    final data = await _postRaw(
      '/ai-chat/threads',
      body: {'title': 'Tro ly AI StaySmart'},
      timeout: AppConstants.aiChatTimeout,
    );
    final createdThreadId = _aiThreadId(data);
    if (createdThreadId == null || createdThreadId.trim().isEmpty) {
      throw const ApiException('AI chat thread response is invalid.');
    }
  ...
  Future<List<Map<String, dynamic>>> fetchSmartRecommendations({
    ...
    final data = await _get(
      ...
      queryParameters: {
        if (_session?.userId != null) 'userId': _session!.userId,
  ...
  Future<List<Map<String, dynamic>>> fetchSimilarHotels({
    required int hotelId,
    int topK = 4,
  }) async {
    final data = await _get(
      '/recommendations/similar/$hotelId',
      queryParameters: {'topK': topK},
    );
    return _recommendationHotels(data)
  ...
  Future<String> uploadCurrentUserAvatar({
    ...
    final avatarUrl = _stringValue(data['avatarUrl']);
    if (avatarUrl == null) {
      throw const ApiException('Invalid avatar upload response.');
    }
    return avatarUrl;
  ...
  List<Map<String, dynamic>> _recommendationHotels(dynamic data) {
    if (data is Map) {
      ...
      if (hotels is List) {
        return hotels
            .map(_mapFrom)
            .where((item) => item.isNotEmpty)
            .map(_mapRecommendationHotel)
            .toList();
      }
    }
    return _listFromData(data).map(_mapRecommendationHotel).toList();
  }

  Map<String, dynamic> _mapRecommendationHotel(Map<String, dynamic> raw) {
    final id = _intValue(raw['id']) ?? 0;
    final ward = _stringValue(raw['ward']);
    final province = _stringValue(raw['province']);
    final location = [ward, province]
        .where((item) => item != null && item.isNotEmpty)
        .join(', ');
    final rating = _numValue(raw['averageRating']) ?? 0;
    final price = (_numValue(raw['avgRoomPrice']) ?? 0).round();
    final score = _numValue(raw['similarityScore']) ?? 0;
    final palette = _hotelPalette(id);
    return {
      'id': id,
      'ownerId': _intValue(raw['ownerId']),
      ...
      'location': location.isEmpty ? 'Dang cap nhat dia chi' : location,
      'subtitle':
          '${_intValue(raw['roomCount']) ?? 0} phong - ${rating.toStringAsFixed(1)} sao',
      'description': 'Goi y phu hop tu StaySmart.',
      'rating': rating,
      'reviews': '${_intValue(raw['bookingCount']) ?? 0} booking',
      'price': price > 0 ? '${_formatCurrency(price)} VND' : 'Xem gia phong',
      'nights': 'Diem goi y ${(score * 100).round()}%',
      'image': '\u{1F3E8}',
      'imageUrl': _resourceUrl(raw['imageUrl']),
      'icon': Icons.apartment_outlined,
      'color': palette.first,
